...
use crate::control::{DETECTION_MULTIPLIER, StreamCancelHandle};
...
///
/// Mirrors [`crate::control::AnchorAttachRequest`] but target semantics and
        ...
        tokio::select! {
            ...
            result = tokio::time::timeout(heartbeat_deadline, transport_rx.recv_async()) => {
                match result {
                    ...
                    Ok(Err(_)) => {
                        let dropped = crate::sender::cached_dropped().clone();
...
/// [`crate::control::create_anchor_attach_handler`]: quick existence check,
/// async `transport.bind().await` outside the shard lock, then atomic slot
/// insertion under the lock.
pub fn create_mpsc_anchor_attach_handler(manager: Arc<AnchorManager>) -> velo_messenger::Handler {
    velo_messenger::Handler::typed_unary_async(
        "_mpsc_anchor_attach",
        move |ctx: velo_messenger::TypedContext<MpscAnchorAttachRequest>| {
...
pub fn create_mpsc_anchor_detach_handler(manager: Arc<AnchorManager>) -> velo_messenger::Handler {
    velo_messenger::Handler::typed_unary_async(
        "_mpsc_anchor_detach",
        move |ctx: velo_messenger::TypedContext<MpscAnchorDetachRequest>| {
            let manager = manager.clone();
            async move {
                let req = ctx.input;
                let (_, local_id) = req.handle.unpack();
...
pub fn create_mpsc_anchor_cancel_handler(manager: Arc<AnchorManager>) -> velo_messenger::Handler {
    velo_messenger::Handler::typed_unary_async(
        "_mpsc_anchor_cancel",
        move |ctx: velo_messenger::TypedContext<MpscAnchorCancelRequest>| {
            let manager = manager.clone();
            async move {
                let req = ctx.input;
                let (_, local_id) = req.handle.unpack();
