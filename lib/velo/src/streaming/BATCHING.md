...
| `velo_streaming_mux_credit_lost_total` | **Should always be zero.** The same credit where the hand-back was refused, which happens only when the batcher had already taken its last drain. The sender's window for that slot stays short by this much until the slot closes |
