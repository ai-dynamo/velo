...
| `velo_streaming_mux_batcher_wakes_total{source}` | Wakes of the per-peer batcher tasks, by what woke them (`open`, `control`, `frame`, `inlet_closed`, `linger`). One wake can still write more than one batch — a size clamp or an oversized record flushes inline — so this is not a batch denominator, only an attribution of what woke the task |
