...
//! `entry_peer`'s own writers (`collect_touched_grants` on the arrival path
//! and the doorbell, `collect_grants` on the periodic tick, and `fail_slot`),
...
///   close replies the ingress reconcile passes and `fail_slot` produce, the
///   hot one being `collect_touched_grants` on the arrival path and the
///   doorbell, with `collect_grants` on the periodic tick behind it — names a
///   slot the ingress table actually admitted, and the table never holds
///   more than one live generation per index at a time, so none of it is
///   ever refused. But the table's own slot limit does not cap `peers`: a
...
struct ControlState {
    ...
    /// The task has taken its last drain and reads nothing further.
    ///
    /// Set by [`ControlInbox::close`] under the same lock every writer takes,
    /// which is what makes "did my write land before the last read" a
    /// question with an answer: a write that got the lock first is in what
    /// `close` handed back, and one that got it second is refused and told
    /// so. Without it a writer could only check liveness *before* writing,
    /// and a task that has already drained for the last time but not yet
    /// exited looks alive to that check — the reply landed in a map nobody
    /// would read again.
    closed: bool,
...
impl ControlState {
    ...
    ///
    /// Never refused: the ingress reconcile passes — `collect_touched_grants`
    /// on the arrival path and the doorbell, `collect_grants` on the periodic
    /// tick — and `fail_slot`, the only writers, name a slot the ingress
    /// table actually admitted. See `ControlState`'s struct doc for `peers`'s
    /// real bound between drains and why leaving it unrefused is still safe.
    /// An `OpenSlot` the ingress rejects outright never comes through here,
    /// whether or not its id happens to match a slot admitted under a
    /// different `OpenSlot` — see `ControlState::reject`.
...
impl ControlInbox {
    ...
    /// Take everything pending and refuse every write from here on.
    ///
    /// The task's last read. Anything that landed since the previous `take`
    /// comes back to be applied before the final flush; anything a writer
    /// posts after this is refused — see [`ControlInbox::reply`] — so the
    /// writer re-resolves onto the batcher that replaced this one rather
    /// than into a map nobody reads. Idempotent: a second close finds
    /// nothing pending, because nothing can land in between.
    pub(super) fn close(&self) -> Option<DrainedControl> {
        let mut state = self.lock();
        state.closed = true;
        if state.is_idle() {
            return None;
    ...
    /// Queue control records to send back to the peer.
    ///
    /// `false` means the task has taken its last drain (see
    /// [`ControlInbox::close`]) and nothing was queued; the caller re-resolves
    /// the peer's batcher and posts there. One lock hold for the whole slice
    /// rather than one per record, so the answer covers all of it: a close
    /// landing between two records of one call would otherwise deliver the
    /// first and refuse the second, and the caller re-posting the slice to a
    /// fresh batcher would grant the first one's credit twice.
    pub(super) fn reply(&self, records: &[ReplyRecord]) -> bool {
        self.mutate(|state| {
            for record in records {
                match *record {
                    ...
                    ReplyRecord::CloseSlot { slot, reason } => {
                        state.entry_peer(slot).close.get_or_insert(reason);
                    }
                    ReplyRecord::RejectSlot { slot, reason } => state.reject(slot, reason),
                }
            }
        })
    }
    ...
    /// Apply one change under the lock and wake the task — unless the task
    /// has taken its last drain, in which case nothing is applied and the
    /// answer is `false`.
    fn mutate(&self, apply: impl FnOnce(&mut ControlState)) -> bool {
        let refused = {
            let mut state = self.lock();
            if state.closed {
                return false;
...
mod tests {
    ...
    fn replies_for_held_slots_are_never_refused() {
        ...
        for index in 0..(OLD_CAP + 5_904) {
            inbox.reply(&[ReplyRecord::CreditUpdate {
                slot: slot(index, 0),
                delta: 1,
            }]);
        }
        ...
        inbox.reply(&[ReplyRecord::CloseSlot {
            slot: slot(OLD_CAP + 5_903, 0),
    ...
    fn a_flood_of_bogus_open_rejections_is_capped() {
        ...
        for index in 0..flood {
            inbox.reply(&[ReplyRecord::RejectSlot {
                slot: slot(MAX_INGRESS_SLOTS_PER_PEER as u32 + index, 0),
        ...
        inbox.reply(&[ReplyRecord::RejectSlot {
            slot: slot(MAX_INGRESS_SLOTS_PER_PEER as u32, 0),
            reason: CloseReason::UnknownSlot,
        }]);
    ...
    /// `close` is the last read: what landed before it comes back, what
    /// lands after is refused, and a writer can tell which side it was on.
    ///
    /// The reply path used to check liveness before writing, and a task that
    /// had taken its last drain but not yet exited passed that check — the
    /// reply landed in a map nobody read again. Deciding it under the lock
    /// the drain takes leaves no third case.
    #[test]
    fn close_hands_back_what_landed_and_refuses_what_follows() {
        let inbox = ControlInbox::default();
        let id = slot(3, 0);
        assert!(
            inbox.reply(&[ReplyRecord::CloseSlot {
            ...
            "before the last read, a reply is taken"
        );

        let drained = inbox.close().expect("the close hands back what landed");
        assert_eq!(
            drained.peers[&id.raw()].close,
            Some(CloseReason::UnknownSlot)
        );

        assert!(
            !inbox.reply(&[ReplyRecord::CreditUpdate { slot: id, delta: 1 }]),
            "after the last read, a reply is refused so its writer re-resolves"
        );
        assert!(inbox.take().is_none(), "and nothing was queued by it");
        assert!(
            inbox.close().is_none(),
            "a second close finds nothing: nothing can land after the first"
        );
    }
}
