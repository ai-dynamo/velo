...
async fn the_fence_lifts_when_the_admission_answers_into_a_full_control_map() {
    ...
    // A delta from here rather than an assertion that this is zero, because
    // resolutions live in their own map (see `MAX_PENDING_CONTROL`'s doc) and
    // can never be refused regardless of what else lands beforehand — the
    // filler's own `OpenSlot` resolved synchronously and reports nothing (see
    // `fire_singleton`'s doc: an admission that never fenced has no fence to
    // lift, so its watcher writes nothing), but the delta is what makes the
    // property under test independent of that.
