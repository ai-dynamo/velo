...
//!
//! `control.rs`'s own unit tests pin the bound `ControlState::reject`
...
fn a_bogus_open_slot_flood_reaches_control_as_a_bounded_reject() {
    ...
    // Dispatched exactly as `deliver_batch` does through `BatcherHandle::reply`.
    let inbox = ControlInbox::default();
    assert!(
        inbox.reply(&outcome.replies),
        "an open inbox takes the slice"
    );
