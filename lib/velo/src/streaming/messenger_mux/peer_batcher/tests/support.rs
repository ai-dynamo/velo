...
pub(super) struct StalledHarness {
    pub(super) handle: Arc<BatcherHandle>,
    /// The peer itself, so a test can read what was *offered* to it and what
    /// admission refused to take — the only positive facts about a parked
    /// batcher that no metric on this side can supply.
    pub(super) transport: Arc<StallingTransport>,
