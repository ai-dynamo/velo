...
use crate::events::status::{EventStatus, Generation};
...
mod tests {
    ...
    fn entry_status_for_poisoned() {
        ...
        let poison = Arc::new(crate::events::status::EventPoison::new(handle, "test"));
        entry
    ...
    fn entry_poison_reason() {
        ...
        let generation = entry.begin_generation().unwrap();
        let handle = entry.key().handle(0, generation);
        let poison = Arc::new(crate::events::status::EventPoison::new(handle, "oops"));
        entry
            .finalize_completion(generation, CompletionKind::Poisoned(poison))
            .unwrap();
    ...
    fn try_to_poison_pending_succeeds() {
        ...
        let generation = entry.begin_generation().unwrap();
        let handle = entry.key().handle(0, generation);
        let poison = Arc::new(crate::events::status::EventPoison::new(handle, "boom"));
    ...
    fn try_to_poison_already_poisoned_is_idempotent() {
        ...
        let generation = entry.begin_generation().unwrap();
        let handle = entry.key().handle(0, generation);
        let poison = Arc::new(crate::events::status::EventPoison::new(handle, "first"));
        match entry.try_to_poison(generation, poison).unwrap() {
            PoisonOutcome::Poisoned => {}
            PoisonOutcome::AlreadyPoisoned => panic!("expected Poisoned on first call"),
        }
        let poison2 = Arc::new(crate::events::status::EventPoison::new(handle, "second"));
    ...
    fn try_to_poison_already_triggered_returns_error() {
        ...
        entry
            ...
            .unwrap();
        let handle = entry.key().handle(0, generation);
        let poison = Arc::new(crate::events::status::EventPoison::new(handle, "too late"));
    ...
    fn try_to_poison_invalid_generation() {
        ...
        let poison = Arc::new(crate::events::status::EventPoison::new(handle, "wrong gen"));
    ...
    fn try_to_poison_wakes_waiters() {
        ...
        let handle = entry.key().handle(0, generation);
        let poison = Arc::new(crate::events::status::EventPoison::new(handle, "wake test"));
