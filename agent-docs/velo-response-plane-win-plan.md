...
## Addendum 2026-09-06 night: W2(e) closed; the inline receiver stays

`t3-now2e72` (results addendum of this night) puts the tail cost on PR #85: without it, exhaustion and ITL p99 return to their `t3-final72` levels on the same nodes, and CPU per request does not change either way. #85 is closed with the measurement in its body. The adapter's inline receiver stays. The integration branch reverts the #85 merge. Nothing in the grant path is left to take: after W2(d) a grant rides an existing batch and costs about 0.1 ms/req in total. Next, in order: the starved-slot urgent grant on #83 (the credit tail is the mechanism behind velo's ITL spikes), then TTFT p99 at 72 workers, then the optional hop-chain CPU items, each with a same-matrix tail check.

## Addendum 2026-09-06 night: the urgent grant is sized and not built

The follow-up left on PR #83 (stage a starved slot's credit grant urgent instead of waiting the reply window) was sized before it was designed. The per-token data of twelve reps (results addendum of this night) shows no excess of long inter-token gaps for velo3 over mux18p at a matched draw; the credit tail exists in the last four token positions at 76 to 106 gaps per rep of 63.5 million. A starved sender waits under 2 ms, about 300 times per rep. The change also carries the risk PR #81 removed: an urgent reply per wake gives one batch per wake on the frontend, and the arrival path has no rate floor. It is not built. If a later measurement reopens it, the first step is a stall histogram on the sender (`velo_streaming_slot_credit_stall_seconds`, one `Instant` per starvation episode), not the mechanism.

What remains open on the bar is first-token p99 at 72 workers: 40 to 70 ms behind mux18p at a matched draw on every matrix since the one-runtime fix, wider at the heaviest draws. The diagnosis of 2026-09-05 put p99 with the hot mocker process; that claim has not been tested per request since W3 and the one-runtime fix. Next: decompose first-token p99 per request at a matched draw (the A/B/C join of the diagnosis, `ttft-join-segments`) on `t3-now2e72`, and name which segment carries the 40 to 70 ms before any mechanism is proposed.

## Addendum 2026-09-10: the p99 clause is met in steady state; the bar is met at a matched draw

The per-request decomposition of first-token p99 on `t3-now2e72` (results addendum of this date, diagnosis section 10) shows the reported p99 is the run's opening burst of 8,192 requests, absorbed by the request plane and the mocker's ingest in both arms alike. In steady state velo3's p99 is ahead of mux18p's at a matched draw (181 and 171 ms against 229; 265 against 308), as is its p90. With p50 ahead, throughput equal at a matched draw, zero errors, and CPU recorded rather than required, the bar is met at a matched draw in steady state on this rig.

What changes: the rig's summary reports steady-state TTFT percentiles (requests started 10 s or later into the profiling phase) next to the raw ones, and every verdict from here reads the steady-state columns. The burst itself is a rig property; a concurrency ramp would remove it, and this aiperf version has none. Open items, none of them on the bar: the hop-chain CPU items (optional, each with a same-matrix tail check), the two adapter changes to upstream in dynamo (one runtime, the inline receiver), axum's shutdown-watch re-poll, and the draft PRs #77 to #84 that await a clean review pass and the author's call on the version split.
