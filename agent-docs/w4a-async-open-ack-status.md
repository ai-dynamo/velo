...
## Addendum, 2026-09-10: the meter no longer wraps on an inverted pair

The counter half of the 2026-09-05 follow-up above is done. `note_batch_seq` now keeps `last_batch_seq` as a high-water mark under the RFC 1982 comparison (`batch_seq_is_newer`, no longer `#[cfg(test)]`). A batch behind the mark, which is a duplicate or the earlier half of an inverted pair, is not metered and does not move the mark. An inverted pair therefore costs `velo_streaming_mux_batch_seq_gaps_total` exactly one, for the later batch that arrived first, and not `u32::MAX`. Test: `a_batch_arriving_after_its_successor_is_not_metered_as_a_gap` in `ingress/tests.rs`. The per-class `batch_seq` that would remove that remaining one is still the open follow-up.
