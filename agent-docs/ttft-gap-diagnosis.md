...
Settled by the counters PR #80 added, read from `t3-iso2` rep 1 of velo3 (`w7_batches.py`): every record the frontend sends is a `CreditUpdate` (28,538,265 of them for 258,192 streams, 110 per stream, 31 per batch), and the frontend's batcher wakes 914,572 times for control against 912,329 batches sent, one batch per wake. Nothing else is in the mix: no closes, no data. The wake-to-batch ratio is the mechanism. A credit reply marked its batch urgent, so under the default write-on-admission policy every wake wrote whatever it held; the receiver's batcher wakes once per reply the sweep hands it, and how many replies one wake absorbs is set by how long the previous flush took to be admitted. With the attach round trip gone the frontend's egress carries nothing but these batches, admission is immediate (13 us mean queue wait against 1.4 ms with attach responses in the same queue, `velo_transport_egress_queue_wait_seconds`), and a wake absorbs almost nothing. The fix is `MuxConfig::reply_linger` (PR #81, 1 ms by default): a batch holding nothing but credit replies forms for the window or until the next record that does not wait, so the batcher writes once per window per peer instead of once per reply; the return a sender is owed is delayed by at most the window, `reply_linger / initial_credit` per record. Arms `velo3n` (linger off) and `velo3f` (velo3 with a 500 us data linger on the workers, whose 67 million data records ride 15 to a batch and cost the frontend 53,000 inbound batches per second) are in the rig for the next matrix. This is the largest first-token lever left: recovering the request path returns velo34 to about 48 ms at p50, which is the bar.
...
### 7. The frontend runs two tokio runtimes, and every response record crosses between them

Found while chasing the W2 collapse (section 6's numbers), by reading dynamo's runtime construction and tokio 1.48's scheduler, and confirmed against the profile stacks.

The dynamo frontend process has two 72-worker tokio runtimes on its 72 pinned cores. Runtime A is dynamo's own `Runtime` (primary and secondary are the same runtime); the velo node is built inside it, so the TCP readers, the messenger receive loop, the eight `_stream_batch` lanes, the peer batchers and the sweep all run there. Runtime B is the one `pyo3-async-runtimes` builds lazily on the first `future_into_py`, because the bridge initialisation in the Python bindings sits in an `.or_else` arm that can never run; the HTTP handlers, the reader pumps (spawned from the request path) and the adapter's response consumers all run there. That is 144 worker threads on 72 cores, both named `tokio-runtime-worker`, which is why the profile could not tell them apart.
...
### 9. What "one wake per burst" turned out to be

A read-only trace of both frontends' per-record paths (2026-09-06, hyper 1.11, tokio 1.48) corrected section 8's reading of the HTTP bucket. hyper drains the body until Pending inside one `poll_write` and flushes once per dispatcher poll, and below the mailbox both arms run the same code, so velo cannot make hyper flush twice for a record. Neither arm flushes once per record: velo about 0.9 times, mux18p about 0.54, because mux18p's body task runs further behind its arrival stream and packs more events per write. That packing cannot be manufactured without holding records, which is a linger, so there is no "coalesce the SSE flushes" lever. What velo's chain does have and mux18p's does not is two extra task wakes and one extra channel per record: the reader pump relay, and the adapter's consumer task with its 64-deep mailbox.

Two changes follow, and one is ruled out. The adapter can hand the anchor to the pipeline as an inline stream polled by the connection task, the way `MuxStreamReceiver` already is, deleting the consumer task and its mailbox (0.3 to 0.6 ms/req). The reader pump cannot be merged into the anchor's channel: the anchor's channel has other writers (a same-worker attach, detach and finalize, the watchdog's sentinel, MPSC senders), and credit is issued against the mux buffer's sole writer, so that hop stays. And the trace found the largest reverse-path cost of all: since W2(d)'s second cut, a slot is reconciled about once per record, so `take_grant` mints one `CreditUpdate` per drained record, 61.7 million grants for 66 million records in `t3-prof5`, against 28.8 million before W2. A grant once per half window, forced by the periodic sweep, withholds nothing a sender can use and cuts that traffic about a hundredfold.

### 10. First-token p99 per request: the burst at the start, and the hot process in steady state

Results addendum of 2026-09-10 (`t3-now2e72`, six reps, `a9_tail.py`). Every request at or above the reported p99 started within the first 0.2 s of the profiling phase, in every rep of both arms. aiperf opens the phase with all 8,192 credits at once, after a warm-up that does not carry over. The 2,500 requests that define p99 are one third of that burst, and their excess over the median is B (frontend ingress to the worker handler's start) by 400 to 593 ms: the request plane and the mocker's Python ingest queue absorbing 1,024 simultaneous requests per process. The response plane is a minority term at the burst (C excess 157 to 320 ms, the same range in both arms). Section 2's claim that p95 and p99 are the hot process's first tokens was right for the p90-to-p99 band in steady state (59 to 89 percent of it sits on one process with 3,000 to 6,400 in flight) and wrong for p99 as reported, which is the burst.

In steady state (requests started 10 s or later) velo3's p99 is 171 to 265 ms and mux18p's 158 to 308, and at a matched draw velo3 is ahead (181 and 171 against 229; 265 against 308). The steady-state tail is C on the hot process plus A on the frontend's HTTP ingress, 39 to 112 ms, alike in both arms. The rig's summary reports the burst as p99; it will report steady-state percentiles as well. A concurrency ramp in aiperf would remove the burst itself; this aiperf version ramps request rate, not concurrency.
