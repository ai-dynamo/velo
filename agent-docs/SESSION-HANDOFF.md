...
- **Integration branch**: `integration/response-plane-wheel` is 3834c9b plus the #83 review corrections (merged after the final matrices; the wheel in `aiperf-venv-b` is 3834c9b and was not rebuilt for the doc-and-sweep commit). `integration/w2a-only`, `integration/w2d-only` and the worktree `velo-w2donly` are leftovers and can be deleted; `velo-w2d2` is a second checkout of #83's branch name and can go too.
