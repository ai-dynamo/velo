...
- **Next**: push the W7 branches when their gate is green; when the wheel lands, `smoke-all.sh` on every velo arm with the new wheel, then W2 (d) touched-slot reconcile as the first PR of the profile's order.
