...
- Clamp warnings: 73 per velo-quic rep = 33 on the frontend (32 server sockets + 1 dial socket) + 5 per mocker (4 + 1). Dynamo's plane logs none, so its effective buffer size is not recorded.
...
- **velo-quic does not lose to velo-tcp.** At one holder (two velo-quic reps against one velo-tcp rep) and at two holders (one against one), req/s, first-token p50/p90/p99, ITL, and CPU per request agree within the spread between reps. The cells are n = 1-2, so the 1-2 ms first-token and 0-5% CPU differences are not findings. Step 6 (profile the frontend if velo-quic loses clearly) did not trigger.
...
- **velo-quic against dynamo-quic.** At two holders velo-quic moved 2,742 req/s against 2,328-2,496, with first-token p99 112 ms against 123-148, ITL p99 45 against 51-68, and CPU 13.1 against 14.0-14.4 ms/request. At one holder the direction is the same. dynamo-quic's frontend received about ten times the datagrams and dropped 16-40 times as many. Inference, not checked: with the batch interval at 0, Dynamo's plane sends roughly one packet per token record, where velo's mux packs many records into each frame. Its shipped 5 ms interval would change that and was not measured.
- **Dynamo's QUIC works on this cluster now.** With quinn 0.11.12 / quinn-proto 0.11.18 and 32 endpoints it served 3 x 250,000 requests with zero client errors at `rmem_max` 212992. Earlier failures on this cluster are attributed to quinn-proto 0.11.17's `too many gaps in stream buffer` and the clamped buffer; this sweep did not reproduce the old build to separate the two.
