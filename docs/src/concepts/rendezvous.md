...
With both knobs on, a peer that is registered but never used is wired up once and closed about one timeout (see the close window above) after `ucp_ep_create` returns. The TCP wireup itself finishes later, in the background.
