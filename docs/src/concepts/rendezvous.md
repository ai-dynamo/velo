...
Patterns where the reaper is safe: peers idle in both directions, and send-only fan-out. It is unsafe in three patterns:

- Peers that this instance only health-probes. Each probe creates an endpoint, and each reap costs the peer one lost Message (or a ping that times out).
