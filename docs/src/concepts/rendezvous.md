...
`UcxTransportBuilder::ep_idle_timeout(Some(d))` closes UCX endpoints that nothing used for `d`. It is **off by default and experimental.** Values below 500 ms rise to 500 ms, which is about 35 times the endpoint wireup measured on a worker that has already created an endpoint.

- "Used" means both directions. Our sends, GETs, pings, and eager wireup stamp the endpoint. Inbound Messages and pings stamp it too. Velo sends those two with UCX's REPLY flag, so UCX hands the receive callback the same endpoint pointer that `ucp_ep_create` returned. Responses, Events, Acks, Pongs, and ShuttingDown echoes are sent without the flag, so they do not stamp the endpoint.
