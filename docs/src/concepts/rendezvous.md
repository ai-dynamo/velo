...
- "Used" means both directions. Our sends, GETs, pings, and eager wireup stamp the endpoint. Every frame the peer sends us stamps it too, of any kind. Each frame header starts with the sender's 8-byte incarnation, and the receiver stamps the endpoint to the peer with that incarnation.
...
**Closing an endpoint costs the peer.** UCX pairs endpoints by remote worker. The peer's own endpoint back to us rides the same connection. After a reap, the peer's next Message or ping to us is admitted and silently lost, with no error at either end. Its Responses, Events, and Acks still arrive (measured: 8 of 8 in every run). The likely reason, inferred from the UCX source and a measurement but not proven: a REPLY-flagged frame carries an endpoint id for the reply path, and UCX drops a frame whose endpoint id no longer resolves. UCX keepalive (about 20 s) then declares the peer's endpoint failed, and the frame after that arrives. The cost is one lost Message or ping and up to one keepalive interval per reaped endpoint. The disruption self-heals.

Patterns where the reaper is safe: peers idle in both directions, and send-only fan-out. It is unsafe in two patterns:

- Peers that this instance only health-probes. Each probe creates an endpoint, and each reap costs the peer a frame.
- Heavy fan-in. The progress thread keeps at most eight refreshes until it drains them, at most once per loop pass. If more than eight frames, from all peers together, arrive before a drain, the oldest are overwritten and those frames do not refresh their endpoints. A peer that is sending can then be reaped as if it were idle.
