...
- Peers that stream Responses or Events back to us for longer than the timeout while this side sends nothing. Those frames do not stamp the endpoint, so it can be reaped mid-stream. The stream still arrives, but the peer then loses its next Message or ping to us. Use a timeout longer than the longest such stream, or leave the reaper off.
