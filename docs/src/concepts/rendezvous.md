...
- The progress thread reads the clock a few times per pass, not once per use: at the start of the pass, after `ucp_worker_progress` returns, and after each `ucp_ep_create`. An inbound Message or ping is stamped after it arrives, which keeps the endpoint open a little longer. A send can be stamped up to one command drain before it is posted. With the reaper off, the progress thread does not read the reaper's clock per pass.
...
- Peers that stream Responses or Events back to us for longer than the timeout while this side sends nothing. Those frames do not stamp the endpoint, so it is reaped mid-stream and the peer loses a frame. Use a timeout longer than the longest such stream, or leave the reaper off.
...
With both knobs on, a peer that is registered but never used is wired up once and closed one timeout after `ucp_ep_create` returns. The TCP wireup itself finishes later, in the background.
