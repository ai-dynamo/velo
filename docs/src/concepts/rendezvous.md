...
- "Used" means both directions. Our sends, GETs, pings, and eager wireup stamp the endpoint. Inbound Messages and pings stamp it too. Velo sends those two with UCX's REPLY flag, so UCX hands the receive callback a reply endpoint. For a peer that we hold an endpoint to, that reply endpoint is the same pointer that `ucp_ep_create` returned (measured separately, and asserted by a test). Responses, Events, Acks, Pongs, and ShuttingDown echoes are sent without the flag, so they do not stamp the endpoint.
- An endpoint is idle only when no send posted on it is in flight and none has completed for the timeout. The reaper counts the sends in flight on each endpoint, and an asynchronous completion of the last one restarts the idle clock. A send that completes inline, while it is posted, records no completion time; the stamp taken when it was posted stands in for it. Replies on the endpoint that UCX hands the receive callback (Pongs and ShuttingDown echoes) are not counted. The inbound frame that caused each reply refreshes the endpoint in the same loop pass, before the reaper scans, unless more than eight REPLY-flagged frames arrived before that refresh was drained.
- The reaper never closes an endpoint while an RDMA operation to that peer is outstanding. A GET's completion does not restart the idle clock, as a send's does. Only the stamp from posting the GET counts, so after a GET slower than the timeout the endpoint can be closed at the next scan.
...
The reaper is meant for peers idle in both directions. That case still carries the cost above, whichever side speaks first, when the peer keeps its endpoint to us: if we speak first, our send arrives with no error, and the peer still loses its next Messages to us, and, by the same inferred mechanism, its pings. If the peer also runs the reaper and has closed its own endpoint, its next send creates a fresh one; that case is not measured. Send-only fan-out costs nothing, because no peer uses a path back. The reaper is unsafe in three patterns:

- Peers that this instance only health-probes. Each probe creates an endpoint, and each reap costs the peer its next Messages and pings to us, if it sends any before keepalive fails its endpoint.
- Peers that stream Responses, Events, or Acks back to us for longer than the timeout while this side sends nothing. Those frames do not stamp the endpoint, so it can be reaped mid-stream. The stream still arrives, but the peer loses its next Messages and pings to us, if it sends any before keepalive fails its endpoint. Use a timeout longer than the longest such stream, or leave the reaper off.
- Heavy fan-in. The progress thread keeps at most eight refreshes until it drains them, at most once per loop pass. If more than eight Messages and pings, from all peers together, arrive before a drain, the oldest are overwritten and those frames do not refresh their endpoints. A peer that is sending can then be reaped as if it were idle.
