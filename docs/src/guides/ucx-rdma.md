...
| `tcp_ep ... recv(-1) failed: Input/output error` on a peer | The endpoint idle reaper closed an endpoint. | None, if `ep_idle_timeout` is on. See [Endpoint idle reaper](../concepts/rendezvous.md#endpoint-idle-reaper). |
