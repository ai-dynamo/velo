...
`prebind_anchor` returns `None` whenever there is nothing to mint, and the caller never has to treat that as a failure: a worker that gets no ticket calls `attach_anchor` exactly as before. Only the no-`messenger-mux` case is silent, because it is configuration — the rollback — rather than a mistake. Every other `None` — an MPSC anchor, another node's handle, an anchor already attached or pre-bound, or one since removed — is a handle this node was asked to pre-bind and cannot. Those are logged at debug and recorded as `outcome="error"` under `velo_streaming_anchor_operations_total{operation="prebind"}`, so call `prebind_anchor` only on the SPSC anchors this node created. `prebind_anchor` must be called from a runtime context — it spawns the reader pump and the bind's accept-window task, exactly as an ordinary attach does. A ticket the worker never opens is not held forever: the bind's 60 s accept window reclaims it, the consumer's anchor sees `SenderDropped`, and the reap counts once under `velo_streaming_unclaimed_bind_reaped_total` — the series to watch for tickets minted and never opened.
...
```toml
[dependencies]
velo = { version = "0.13", features = ["distributed-tracing"] }
```
